...
const adaptors = fs.readdirSync(adaptorsDir).filter(f => fs.statSync(path.join(adaptorsDir, f)).isDirectory());

const getViteConfig = (name, framework) => {
    let plugins = "dts({ include: ['src'], insertTypesEntry: true })";
    let imports = "import dts from 'vite-plugin-dts';";
    let external = "['react', 'react-dom', 'vue', (id) => id.startsWith('@sxo/')]";
...
for (const adaptor of adaptors) {
    ...
    if (fs.existsSync(packageJsonPath)) {
        ...
        const className = adaptor.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('');
