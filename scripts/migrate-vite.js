...
const viteConfigTemplate = (name) => `import { defineConfig } from 'vite';
...
themes.forEach(theme => migratePackage(path.join(packagesDir, theme)));

// Components
const components = ['component-admin', 'component-icons', 'component-table'];
components.forEach(comp => migratePackage(path.join(packagesDir, comp)));

// Others
const others = ['sxo-engine', 'sxo-ui', 'sxo-design'];
others.forEach(other => migratePackage(path.join(packagesDir, other)));

// Adaptors
const adaptors = fs.readdirSync(adaptorsDir);
adaptors.forEach(adaptor => migratePackage(path.join(adaptorsDir, adaptor)));
